...
class MedicationManager:
    ...
    async def load_medications(self):
        ...
        if not self.supabase_url or not self.supabase_key:
            logger.warning("Supabase not configured, loading from file")
            self._load_medications_from_file()
            return
        
        try:
            async with aiohttp.ClientSession() as session:
                url = f"{self.supabase_url}/rest/v1/medications?active=eq.true"
                headers = {
                ...
                async with session.get(url, headers=headers) as resp:
                    if resp.status == 200:
                        ...
                        logger.info(f"Loaded {total} medications for {len(self.medications)} patients")
                    else:
                        logger.error(f"Failed to load medications: {resp.status}")
                        self._load_medications_from_file()
